...
import os, shutil, base64, json, time
...
class Block():
	...
	def __del__(self):
		#delete data files after block is deleted
		print 'block', self.folder, ': data deleted'
	...
	def new_item(self, resource, hash, item):
		with self.LOCK:
			...
			if hash in self.header[resource]:
				...
				return True
		try:
			self.data_file = open(self.folder+hash, 'w')
	...
	def create_block(self, timestamp):
		...
		if len(self.header) > 0:
			...
			hash = self.CRYPTO.hash('{"previousBlocKHash":"'+self.previous_hash+'","date":'+str(timestamp)+',"header":'+header+'}')
			signature = self.CRYPTO.sign(hash, 'base64')
			meta = '{"blockHash":"'+hash+'","previousBlockHash":"'+self.previous_hash+'","date":'+str(timestamp)+',"workerNode":"'+self.CONFIG['id']+'","signature":"'+signature+'","header":'

			try:
				block = open(self.CONFIG['blocks_dir']+hash+Globals.BLOCK_EXTENSION, 'wb')
				block.write(meta)
				block.write(header)
				block.write('}')
